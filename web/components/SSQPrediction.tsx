    ...
    const loadPredictions = async () => {
        ...
        try {
            const res = await fetch(
                `${API_BASE_URL}/api/analysis/ssq/recommend?lookback=${lookback}&num_sets=${numSets}&aggregation=all`
            );
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-foreground">双色球 - 时间序列预测</h2>
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                            ...
                                            {pred.error ? (
                                                ...
                                                <div className="flex flex-wrap items-center gap-2">
                                                    <div className="flex gap-1.5">
                                                        {pred.red?.map((n, i) => (
                                                            <div key={i} className="ball ball-red text-xs" style={{ width: 26, height: 26 }}>
                                                                {n.toString().padStart(2, "0")}
                                                            </div>
                                                        ))}
                                                    </div>
                                                    <span className="text-muted-foreground text-xs">+</span>
                                                    <div className="ball ball-blue text-xs" style={{ width: 26, height: 26 }}>
                                                        {pred.blue?.toString().padStart(2, "0")}
                                                    </div>
                                                </div>
                                            )}
                                        </div>
